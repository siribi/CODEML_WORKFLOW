...
#----------------------------------
sub  sort_alignment{
#	input: alignment and format
#	output: alphanumeric sorted alignment in Fasta format
	my $inMsaFile=shift;
	my $format=shift;
	#my $Depth=check_msa_licit_and_size($inMsaFile,$format);
	open (OUT,">$inMsaFile.Sorted") || return ("Can't open '$inMsaFile.Sorted' $!");
	my $in  = Bio::AlignIO->new( '-format' =>  $format, -file => $inMsaFile) or return ("Can't open $inMsaFile: $!");
	my $aln = $in->next_aln;
	$aln->verbose(1); 
	...
	for my $seq_id ( sort bySeqName keys %aln) {
		my $seq=$aln{$seq_id};
		$seq =~ s/(.{60,60})/$1\n/g ;
...
sub bySeqName {
	if (looks_like_number($a) and  looks_like_number($b))
	{
	...
	elsif ($a=~/seq(\d+)/) 
   	{
		...
		{
			my $s2=$1;
			return $s1<=>$s2; # in case it is name in HoT format
		}
	}
	else
	{
		return  fc($a)  cmp  fc($b);
	}
}
#----------------------------------
...
#----------------------------------
    ...
    my $check_sequence_licit=shift; #OPTIONAL
    my $ans = "";
    if ($check_sequence_licit eq ""){$check_sequence_licit="yes";}
    ...
    if ($msa_format eq "fasta" or $msa_format  eq "gcg" or $msa_format eq "pir"){
        ...
        eval{
            while ( $seq = $msa_fh->next_seq() ) { # returns a Bio::Seq sequence object
                $read_seq++;
                $ans = &check_sequence_licit($seq->seq()) if ($check_sequence_licit eq "yes");
                last if $ans ne "";
...
#----------------------------------
sub convert_msa_format{    
# read the MSA, returns a hash with all the sequences id and their corresponding sequences
# input : 1. full path for MSA, 2. in MSA format, 3. full path to an empty file 4. out MSA format
# outputs: ERROR:# 
# ('user_error',"exception") - If an exception found - the SeqIO and AlighIO may through an exception
# ('user_error',"could not read msa") - the MSA was not opened for some reason
# ('user_error',"duplicity <sequence id>" - in case the same sequence name was found more than once
...
#----------------------------------
    ...
    my $infile = shift;
    my $infileformat = shift;
    my $outfile = shift;
    my $outfileformat = shift;
   
    eval{
    	my $in_obj  = Bio::AlignIO->new(-file => $infile ,
        	                     -format => $infileformat);
    	my $out_obj = Bio::AlignIO->new(-file => ">$outfile",
                                     -format => $outfileformat);
    	while ( my $aln = $in_obj->next_aln() ) { $out_obj->write_aln($aln); }
    };
    if( $@ ) { #Caught exception
            @ret =  ('user_error',"exception");
    }
    # BioPerl Add in the end /1-129 so we need to remove it
    unless (open (MSA,$outfile)) {return "Can't Open $outfile for reading $!\n";}
    my @msa = <MSA>;
    close (MSA);
    unless (open (MSA, ">$outfile")) {return "Can't Open $outfile for writing $!\n";}
    foreach my $line (@msa)
    {
    	my $length;
	my $str="";
	if ($line=~/(\/[0-9]+-[0-9]+)/){$length=length($1)};
	if ((defined $length) and ($outfileformat eq "clustalw")){for (my $i=1;$i<$length;$i++) {$str.=" "};}
